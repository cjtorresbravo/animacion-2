...
(function () {
  ...
  function createRuntime(host, svgMarkup, project, hiddenPartIds) {
    ...
    function seek(frame) {
      ...
      applyFrame(svg, project, currentFrame, hiddenPartIds, visualIdToPartId, nestedVisualTransformIds);
    }
  ...
  function applyFrame(svg, sourceProject, frame, hiddenPartIds, visualIdToPartId, nestedVisualTransformIds) {
  ...
  function buildNestedVisualTransformIds(parts, elementsById) {
    const skip = new Set();

    parts.forEach((part) => {
      const visualIds = Array.from(
        new Set(
          Array.isArray(part.visualIds) && part.visualIds.length > 0
            ? part.visualIds
            : [String(part.selector || '').replace(/^#/, ''), part.id]
        )
      );
      const visualElements = visualIds
        .map((visualId) => [visualId, elementsById.get(visualId)])
        .filter((entry) => Boolean(entry[0]) && Boolean(entry[1]));

      visualElements.forEach(([visualId, element]) => {
        const isNestedWithinSiblingVisual = visualElements.some(
          ([otherVisualId, otherElement]) =>
            otherVisualId !== visualId &&
            otherElement &&
            element &&
            otherElement.contains(element)
        );

        if (isNestedWithinSiblingVisual) {
          skip.add(visualId);
        }
      });
    });
  ...
  function computeGeometry(parts, options) {
    ...
    const applyRecursive = (partId, parentMatrix) => {
      ...
      const pivot = part.bone
        ? transformPoint(worldMatrix, part.bone.pivot.x, part.bone.pivot.y)
        : transformPoint(worldMatrix, part.anchor?.x || 0, part.anchor?.y || 0);
  ...
  function composePartMatrix(part) {
    ...
    if (!bone) {
      const anchor = part.anchor || { x: 0, y: 0 };
      let matrix = identityMatrix();
      matrix = multiplyMatrices(matrix, translateMatrix(state.x, state.y));
      matrix = multiplyMatrices(matrix, translateMatrix(anchor.x, anchor.y));
